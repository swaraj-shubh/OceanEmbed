...
from config import CHANNELS, DEPTHS, END, INTERIM, LAT, LON, START, ZARR
...
LOADERS = {
    "sst": load_sst,
    "sss": _todo("SMAP RSS L3 SSS v4 8-day, PO.DAAC / Earthdata"),
    "sla": _todo("CMEMS SEALEVEL_GLO_PHY_L4_MY_008_047"),
    "cur_u": _todo("OSCAR v2.0 total current u, PO.DAAC / Earthdata"),
    "cur_v": _todo("OSCAR v2.0 total current v, PO.DAAC / Earthdata"),
...
if __name__ == "__main__":
    ...
    p.add_argument("--check-sst", action="store_true", help="verify the SST channel only")
    a = p.parse_args()
    if a.check_sst:
        days = pd.date_range(a.start, a.end, freq="D")
        da = load_sst(days)
        assert da.shape == (len(days), len(LAT), len(LON)), da.shape
        assert bool(((da > -2) & (da < 36)).any()), "no valid SST"
        land = np.isnan(da).all("time")
        print(f"SST ok: {da.shape}, {float(land.mean()) * 100:.1f}% always-land cells, "
              f"{float(da.mean()):.2f} degC mean, "
